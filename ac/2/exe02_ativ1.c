// pratica1.c
// Objetivo: estressar CPU/cache, permitir variações (FP vs int, com/sem desvio, stride)
// e medir tempo sem perf (timer portátil).
...
static float kernel_float_branch(const float *A, const float *B, float *C, int N, int STRIDE, int REPEAT) {
...
static float kernel_float_branchless(const float *A, const float *B, float *C, int N, int STRIDE, int REPEAT) {
    const float thresh = 0.5f;
    float acc = 0.0f;
    for (int r = 0; r < REPEAT; ++r) {
        for (int i = 0; i < N; ++i) {
...
static long long kernel_int(const int *A, const int *B, int *C, int N, int STRIDE, int REPEAT) {
...
int main(int argc, char **argv) {
    if (argc < 5) {
        ...
        fprintf(stderr, "  MODE: 0=float+desvio | 1=float sem desvio | 2=int (aprox.)\n");
        return 1;
    }
