...
int compareOwo(const Show *a, const Show *b) {
...
void insertion(Show *aaaaaaaaaaaaaaaa, int n, int k) {
  for (int i = 1; i < n; i++) {
    Show curr = aaaaaaaaaaaaaaaa[i];
    int j;

    for (j = k - 1; j >= 0; j--) {
      if (compareOwo(&curr, &aaaaaaaaaaaaaaaa[j]) < 0) {
        if (j + 1 < k)
          aaaaaaaaaaaaaaaa[j + 1] = aaaaaaaaaaaaaaaa[j];
      } else
        break;
    }

    if (j + 1 < k) {
      aaaaaaaaaaaaaaaa[j + 1] = curr;
    }
  }
}
