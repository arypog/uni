import java.util.Arrays;
import java.util.ArrayList;
import java.util.Scanner;
import java.util.regex.*;
import java.io.File;
import java.io.FileNotFoundException;
import java.time.LocalDate;
import java.time.format.DateTimeFormatter;

class Show {
  public   String   id;
  public   String   type;
  public   String   title;
  public   String   director;
  private  String[] cast;
  private  String   country;
  //  private  Date     date_added; // nao vai acontecer
  public   String   date_added;
  public   int      release_year;
  private  String   rating;
  public   String   duration;
  private  String[] listed_in;
  private  String   description;
  private  int      size = 12;

  void setShow(String[] parts) {

    this.id           = (parts[0].isEmpty()) ? "NaN" : parts[0];
    this.type         = (parts[1].isEmpty()) ? "NaN" : parts[1];
    this.title        = (parts[2].isEmpty()) ? "NaN" : parts[2];
    this.director     = (parts[3].isEmpty()) ? "NaN" : parts[3];
    this.cast         = (parts[4].isEmpty()) ? new String[] {"NaN"} : sort(parts[4].split(", "));
    this.country      = (parts[5].isEmpty()) ? "NaN" : parts[5];
    this.date_added   = (parts[6].isEmpty()) ? "March 1, 1900" : parts[6];
    this.release_year = (parts[7].isEmpty()) ? -1 : Integer.parseInt(parts[7]);
    this.rating       = (parts[8].isEmpty()) ? "NaN" : parts[8];
    this.duration     = (parts[9].isEmpty()) ? "NaN" : parts[9];
    this.listed_in    = (parts[10].isEmpty()) ? new String[] {"NaN"} : parts[10].split(",");
    this.description  = (parts[11].isEmpty()) ? "NaN" : parts[11];
  }

  void printShow() {
    System.out.print(
      "=> "  + this.id +
      " ## " + this.title +
      " ## " + this.type +
      " ## " + this.director +
      " ## " + Arrays.toString(this.cast).replaceAll("  ", " ") +
      " ## " + this.country +
      " ## " + this.date_added +
      " ## " + this.release_year +
      " ## " + this.rating +
      " ## " + this.duration +
      " ## " + Arrays.toString(this.listed_in).replaceAll("  ", " ") +
      " ##\n"
    ); 
  }

  String[] sort(String[] parts) {
    for (int i = 0; i < parts.length; i++) {
      for (int j = 0; j < parts.length - 1; j++) {
        if (parts[j].compareTo(parts[j + 1]) > 0) {
          String tmp = parts[j]; 
          parts[j] = parts[j + 1];
          parts[j + 1] = tmp;
        }
      }
    }
    return parts;
  }

  void readCSVLineRegex(String line) {
  }

  String[] parseCSVLine(String line) {
    int j = 0, k = 0;
    char[] buffer = new char[1000];
    char[] array = line.toCharArray();
    String[] parts = new String[this.size]; 

    boolean inQuotes = false;
    for (int i = 0; i < array.length; i++) {
      char ch = array[i];
      if (inQuotes) {
        if (ch == '\"' && i + 1 < array.length && array[i + 1] == ',') {
          inQuotes = false;
        } else if (ch != '\"') {
          buffer[j++] = ch;
        }
      } else if (ch == ',') {
        parts[k++] = new String(buffer, 0, j);
        j = 0;
      }
      else if (ch == '\"') {
        inQuotes = true; 
      } else {
        buffer[j++] = ch;
      }
    }

    parts[k] = new String(buffer, 0, j);
    return parts;
  }

  void readCSVLine(String line) {
    String[] parts = parseCSVLine(line);
    setShow(parts);
  }
}

public class Main {
  static void sort2(ArrayList<Show> parts) {
    int n = parts.size();
    for (int i = 0; i < n - 1; i++) {
      int min = i;
      for (int j = i + 1; j < n; j++) {
        if (parts.get(j).title.compareTo(parts.get(min).title) < 0) {
          min = j;
        }
      }
      Show temp = parts.get(i);
      parts.set(i, parts.get(min));
      parts.set(min, temp);
    }
  }

  static void sort3(ArrayList<Show> parts) {
    int n = parts.size();
    for (int i = 1; i < n; i++) {
      Show key = parts.get(i);
      int j = i - 1;

      while (j >= 0) {
        Show current = parts.get(j);
        int cmp = current.type.compareTo(key.type);

        if (cmp > 0 || (cmp == 0 && current.title.compareTo(key.title) > 0)) {
          parts.set(j + 1, current);
          j--;
        } else {
          break;
        }
      }

      parts.set(j + 1, key);
    }
  }

  static void heapfy(ArrayList<Show> kaka, int n, int i) {
    int max = i;
    int l = 2 * i + 1;
    int r = 2 * i + 2;

    if (l < n && isGreater(kaka.get(l), kaka.get(max))) {
      max = l;
    }

    if (r < n && isGreater(kaka.get(r), kaka.get(max))) {
      max = r;
    }

    if (max != i) {
      Show tmp = kaka.get(i);
      kaka.set(i, kaka.get(max));
      kaka.set(max, tmp);
      heapfy(kaka, n, max);
    }
  }

  static boolean isGreater(Show a, Show b) {
    int cmp = a.director.compareTo(b.director);
    if (cmp > 0) return true;
    if (cmp < 0) return false;

    return a.title.compareTo(b.title) > 0;
  }

  static void heapSort(ArrayList<Show> kaka) {
    int n = kaka.size();
    for (int i = n/2 - 1; i >= 0; i--)
    heapfy(kaka, n, i);

    for (int i = n - 1; i > 0; i--) {
      Show tmp = kaka.get(0);
      kaka.set(0, kaka.get(i));
      kaka.set(i, tmp);

      heapfy(kaka, i, 0);
    }
  }

  // tygfg
  static void countSort(ArrayList<Show> kaka) {
    int N = kaka.size();

    Show[] inputArray = new Show[N];
    for (int i = 0; i < N; i++) {
      inputArray[i] = kaka.get(i);
    }

    int M = 0;
    for (int i = 0; i < N; i++) {
      M = Math.max(M, inputArray[i].release_year);
    }

    int[] countArray = new int[M + 1];
    for (int i = 0; i < N; i++) {
      countArray[inputArray[i].release_year]++;
    }

    for (int i = 1; i <= M; i++) {
      countArray[i] += countArray[i - 1];
    }

    Arrays.sort(inputArray, (s1, s2) -> s1.title.compareTo(s2.title));

    Show[] outputArray = new Show[N];
    for (int i = N - 1; i >= 0; i--) {
      int year = inputArray[i].release_year;
      outputArray[countArray[year] - 1] = inputArray[i];
      countArray[year]--;
    }

    kaka.clear();

    for (int i = 0; i < N; i++) {
      kaka.add(outputArray[i]);
    }
  }

  public static int getDigit(Show s) {
    Pattern pattern = Pattern.compile("(\\d+)");
    Matcher matcher = pattern.matcher(s.duration);

    if (matcher.find()) {
      return Integer.parseInt(matcher.group(1));
    }

    return 0;
  }

  public static int compare(Show s1, Show s2) {
    int s1N = getDigit(s1);
    int s2N = getDigit(s2);

    if (s1N > s2N) return 1;
    if (s1N < s2N) return -1;

    return s1.title.compareTo(s2.title);
  }

  public static void mergeSort(ArrayList<Show> kaka) {
    if (kaka.size() <= 1) {
      return;
    }

    int mid = kaka.size() / 2;
    ArrayList<Show> left = new ArrayList<>(kaka.subList(0, mid));
    ArrayList<Show> right = new ArrayList<>(kaka.subList(mid, kaka.size()));

    mergeSort(left);
    mergeSort(right);

    merge(kaka, left, right);
  }

  private static void merge(ArrayList<Show> kaka, ArrayList<Show> left, ArrayList<Show> right) {
    int i = 0, j = 0, k = 0;

    while (i < left.size() && j < right.size()) {
      Show leftShow = left.get(i);
      Show rightShow = right.get(j);

      if (compare(leftShow, rightShow) <= 0) {
        kaka.set(k++, leftShow);
        i++;
      } else {
        kaka.set(k++, rightShow);
        j++;
      }
    }

    while (i < left.size()) {
      kaka.set(k++, left.get(i++));
    }

    while (j < right.size()) {
      kaka.set(k++, right.get(j++));
    }
  }

  static void sort2k(ArrayList<Show> parts, int k) {
    int n = parts.size();
    for (int i = 0; i < k - 1; i++) {
      int min = i;
      for (int j = i + 1; j < n; j++) {
        if (parts.get(j).title.compareTo(parts.get(min).title) < 0) {
          min = j;
        }
      }
      Show temp = parts.get(i);
      parts.set(i, parts.get(min));
      parts.set(min, temp);
    }
  }

  static int compareUwU(Show a, Show b) {
    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MMMM d, yyyy");
    LocalDate date1 = LocalDate.parse(a.date_added, formatter);
    LocalDate date2 = LocalDate.parse(b.date_added, formatter);

    int cmp = date2.compareTo(date1);
    if (cmp != 0)
      return cmp;

    return b.title.compareTo(a.title);
  }

  static void qsort(ArrayList<Show> a, int start, int end, int k) {
    if (start >= end) return;

    int piwiotIndex = start + (end - start) / 2;
    Show piwiot = a.get(piwiotIndex);

    int i = start;
    int j = end;

    while (i <= j) {
      while (compareUwU(a.get(i), piwiot) > 0) i++;
      while (compareUwU(a.get(j), piwiot) < 0) j--;

      if (i <= j) {
        Show temp = a.get(i);
        a.set(i, a.get(j));
        a.set(j, temp);
        i++;
        j--;
      }
    }


      if (start < j) qsort(a, start, j, k);
      if (i < end) qsort(a, i, end, k);
  }

  public static void main(String args[]) throws FileNotFoundException {
    //    File f = new File("201/test.csv");
    //File f = new File("201/disneyplus.csv");
    File f = new File("/tmp/disneyplus.csv");
    Scanner sc = new Scanner(f);
    Show[] shows = new Show[1369];

    int i = 0;
    for (; i < shows.length; i++) {
      shows[i] = new Show();
    }
    i = 0;
    sc.nextLine();
    while(sc.hasNextLine()) {
      String line = sc.nextLine();
      shows[i].readCSVLine(line);
      i++;
    }
    sc.close();

    String inputId;
    Scanner scid = new Scanner(System.in);

    // lol
    ArrayList<Show> lol = new ArrayList<Show>();
    while (scid.hasNextLine()) {
      inputId = scid.nextLine();
      if (inputId.equals("FIM")) break;

      for (Show show : shows) {
        if (show.id.equals(inputId)) {
          lol.add(show);
          break;
        }
      }
    }

    qsort(lol, 0, lol.size() - 1, 10);

    for (i = 0; i < 10; i++)
      lol.get(i).printShow();

    sc.close();
    scid.close();
  }
}
