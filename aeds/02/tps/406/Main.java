...
class HashDirectRehash {
  Show[] table;
  int size;

  public HashDirectRehash() {
    this.size = 21;
    this.table = new Show[size];
  }
  ...
  public boolean insert(Show show) {
    ...
    String title = show.title.trim();
    int pos = hash(title);

    for (int i = 0; i < size; i++) {
      int newPos = (pos + i) % size;
  ...
  public void search(String title) {
    ...
    for (int i = 0; i < size; i++) {
      int newPos = (pos + i) % size;
      if (table[newPos] == null) break;
      if (table[newPos].title.equals(title)) {
        System.out.println(" (Posicao: " + newPos + ") SIM");
        return;
      }
    }
...
public class Main {
  ...
  public static void main(String[] args) throws FileNotFoundException {
    ...
    HashDirectRehash hashTable = new HashDirectRehash();
    String line;
